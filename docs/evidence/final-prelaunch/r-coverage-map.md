...
| R3 | A missing runtime dependency cannot look available | `app/api/health/readyRoute.test.ts` (7 assertions incl. the cache-lag one), `PdfToolWorkerHandler.test.ts` *categorizes a MissingDependencyError as missing-dependency*, `tool-matrix.log` live `which`; brief mutations **B**, **O** | PASS |
...
| R23 | Health / readiness truthful | `app/api/health/readyRoute.test.ts` (incl. *reports ready only when EVERY binary resolves*), live on this host: `/api/health/ready` → 503 `toolchain:false` with `soffice` absent; brief mutation **O** | PASS |
...
| R30 | Deployment / rollback smoke | `mutation-C-deployment.md`, `deploymentArtifact.test.ts`, container build proven in `28e377b`; the **rollback rehearsal itself** is Group Q and is not yet run | open at the time of writing |
