...
| R30 | Deployment / rollback smoke | `r30-deploy-rollback.log` — `scripts/r30-rollback-smoke.sh` **8/8 PASS** at `bfed32f`: previous artifact kept, HEAD built and booted, a real job through it, the PREVIOUS artifact booted again and completed its own real job, rolled forward; `rollback-runbook.md`, `deploymentArtifact.test.ts`, `mutation-C-deployment.md` | `PASS` for the standalone path; container path (`docker compose`, image digest, `migrate deploy` in-container) `ENVIRONMENTAL` — no Docker daemon on this host |
