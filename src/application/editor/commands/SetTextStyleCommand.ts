...
import type { TextRunStyle } from '@/src/domain/editor/textContent';

/**
 * Command to set text style for a text object
 * This is a pure immutable command that updates text style properties
 */
export class SetTextStyleCommand implements Command {
  readonly type = 'SetTextStyle';

  constructor(
    public readonly objectId: string,
    public readonly paragraphIndex: number,
    public readonly runIndex: number,
    public readonly before: TextRunStyle,
    public readonly after: TextRunStyle
  ) {}

  /**
   * Applies the command to update text style
  ...
  apply(state: any): any {
    ...
    // Update the content - modify the specific run style
    if (updatedObject.content &&
      ...
      // Update the style in the specific run
      updatedObject.content.paragraphs[this.paragraphIndex].runs[this.runIndex].style = { ...this.after };
   ...
   */
  private rebuildLegacyText(content: any): string {
  ...
  invert(state: any): any {
    ...
    if (updatedObject.content &&
      ...
      // Revert the style in the specific run
      updatedObject.content.paragraphs[this.paragraphIndex].runs[this.runIndex].style = { ...this.before };
