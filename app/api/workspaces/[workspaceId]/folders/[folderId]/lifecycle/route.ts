...
export async function POST(request: NextRequest, { params }: { params: Promise<{ workspaceId: string; folderId: string }> }) {
  ...
  try {
    const folder = await workspaceServices().folders.setLifecycle(actorResult.actor, workspaceId, folderId, parsed.data.state);
    workspaceServices().audit.record({ actorType: "user", actorId: actorResult.actor.userId, organizationId: actorResult.actor.organizationId, action: `folder.${parsed.data.state}`, resourceType: "folder", resourceId: folder.id }).catch(() => {});
